...
const ContactItem = ({ contact, isSelected, onClick }) => {
  return (
    <div 
      ...
      onClick={onClick}
    >
      <div className={`w-14 h-14 rounded-full overflow-hidden mb-2 ${isSelected ? 'ring-2 ring-[#718EBF]' : ''}`}>
        <img 
          src={contact.avatar} 
          alt={contact.name} 
      ...
      <p className={`text-sm text-[#718EBF] ${isSelected ? 'font-bold' : 'font-normal'}`}>{contact.role}</p>
    </div>
  );
};
    ...
    <div className="dashboard-widget">
      ...
      >
        ...
        <div className="flex items-center justify-center mt-10 mx-5 relative">
          <div className="flex gap-10 justify-center w-full px-8">
          ...
          <div className="absolute -right-2 top-1/2 transform -translate-y-1/2">
            <div className="flex items-center justify-center w-10 h-10 rounded-full bg-gray-100 text-[#718EBF] hover:bg-gray-200 cursor-pointer">
        ...
        <div className="flex items-center mt-9 mx-8">
          <span className="text-[#718EBF] text-lg mr-4 whitespace-nowrap">Write Amount</span>
          ...
          <div className="relative flex-1">
            ...
            >
              <div className="flex-1 flex items-center justify-center">
                <input
                  type="text"
                  className="w-full h-full py-3 px-6 bg-transparent border-none focus:outline-none text-gray-800 text-center text-lg"
                  value={amount}
                  onChange={(e) => {
              ...
              <button 
                className={`px-6 h-full text-white flex items-center justify-center ${
              ...
              >
                <span className="mr-2 text-lg">{sending ? 'Sending...' : 'Send'}</span>
                {!sending && <SendIcon />}
              </button>
            </div>
          </div>
