...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="bg-white py-4 px-6 flex items-center justify-between shadow-sm main-navbar" role="navigation" aria-label="Main navigation">
        ...
        <div className={`${isMobile ? 'flex-1 text-center' : 'flex-shrink-0'}`}>
          <h1 className="text-2xl font-bold text-indigo-900" aria-level="1">{getTitle()}</h1>
        </div>
        ...
        <div className="flex items-center space-x-6">
          ...
          {/* User profile */}
          <div className="flex items-center" role="region" aria-label="User profile">
            <div className="w-10 h-10 rounded-full overflow-hidden">
              {loading ? (
                <div className="bg-gray-200 animate-pulse w-full h-full" aria-label="Loading profile"></div>
              ) : (
                <>
      ...
      {isMobile && (
        <div className="bg-white px-4" role="search">
